...
<div class="container py-4">
    <div class="mb-4">
        ...
        <h2 class="fw-bold mb-0 text-dark"><i class="fas fa-edit text-primary me-2"></i>Editar Regra Fiscal: {{ $tax->name }}</h2>
    </div>
    ...
    <div class="card-premium p-4">
        <form action="{{ route('config.taxes.update', $tax->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    <select name="company_id" class="form-select" required>
                        @foreach($companies as $company)
                            <option value="{{ $company->id }}" {{ old('company_id', $tax->company_id) == $company->id ? 'selected' : '' }}>{{ $company->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="code" class="form-control" value="{{ old('code', $tax->code) }}" required>
                </div>

                <div class="col-md-4">
                    <label class="form-label fw-bold">Tipo de Imposto <span class="text-danger">*</span></label>
                    <select name="type" class="form-select" required>
                        <option value="VAT" {{ old('type', $tax->type) == 'VAT' ? 'selected' : '' }}>IVA (Imposto Valor Acrescentado)</option>
                        <option value="RETENTION" {{ old('type', $tax->type) == 'RETENTION' ? 'selected' : '' }}>Retenção na Fonte</option>
                        <option value="STAMP" {{ old('type', $tax->type) == 'STAMP' ? 'selected' : '' }}>Imposto do Selo</option>
                    </select>
                </div>

                <div class="col-md-4">
                    <label class="form-label fw-bold">Taxa (%) <span class="text-danger">*</span></label>
                    <input type="number" step="0.01" min="0" name="rate" class="form-control text-primary fw-bold fs-5" value="{{ old('rate', number_format($tax->rate, 2, '.', '')) }}" required>
                </div>
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="exemption_reason" class="form-control" value="{{ old('exemption_reason', $tax->exemption_reason) }}">
                </div>
                
                <div class="col-12">
                    <div class="form-check form-switch mt-2">
                        <input class="form-check-input" type="checkbox" role="switch" name="is_active" id="isActive" {{ $tax->is_active ? 'checked' : '' }}>
            ...
            <div class="text-end mt-4 pt-3 border-top">
                ...
                <button type="submit" class="btn btn-primary fw-bold px-4" style="border-radius:10px;"><i class="fas fa-save me-2"></i> Atualizar Regra Fiscal</button>
            </div>
        </form>
    </div>
</div>
