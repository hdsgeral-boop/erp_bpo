...
<div class="container-fluid py-4">
    <div class="mb-4">
        ...
        <h2 class="fw-bold mb-0 text-dark"><i class="fas fa-edit text-primary me-2"></i>Editar Perfil</h2>
        <p class="text-muted mb-0">Modificar definições e acessos de {{ $role->name }}.</p>
    </div>
        ...
        <form action="{{ route('admin.roles.update', $role->id) }}" method="POST">
            ...
            <div class="row mb-5">
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="name" class="form-control border-start-0" value="{{ old('name', $role->name) }}" required {{ $role->id === 1 ? 'readonly' : '' }}>
                    </div>
                    @if($role->id === 1)
                    <div class="form-text text-warning"><i class="fas fa-exclamation-triangle"></i> Não é possível alterar o nome do perfil Administrador Principal.</div>
                    @endif
                </div>
            </div>
            ...
            @if($role->id === 1)
                <div class="alert alert-info">
                    <i class="fas fa-info-circle me-2"></i> O perfil <strong>Super Admin</strong> tem automaticamente acesso a todas as funcionalidades do sistema. As permissões abaixo são apenas informativas.
                </div>
            @endif

            <div class="mb-3 d-flex justify-content-end gap-2">
                <button type="button" class="btn btn-sm btn-outline-primary" id="selectAll" {{ $role->id === 1 ? 'disabled' : '' }}><i class="fas fa-check-square me-1"></i> Selecionar Tudo</button>
                <button type="button" class="btn btn-sm btn-outline-secondary" id="deselectAll" {{ $role->id === 1 ? 'disabled' : '' }}><i class="fas fa-square me-1"></i> Limpar Seleção</button>
            </div>
                    ...
                    <div class="module-card">
                        <div class="module-header">
                            ...
                            <div class="form-check m-0">
                                <input class="form-check-input module-checkbox" type="checkbox" data-module="{{ $module }}" {{ $role->id === 1 ? 'checked disabled' : '' }}>
                            </div>
                        </div>
                        <div class="module-body">
                            ...
                            <div class="form-check permission-item">
                                <input class="form-check-input perm-checkbox module-{{ $module }}" type="checkbox" name="permissions[]" value="{{ $perm->name }}" id="perm_{{ $perm->id }}"
                                    {{ (is_array(old('permissions')) && in_array($perm->name, old('permissions'))) || in_array($perm->id, $rolePermissions) || $role->id === 1 ? 'checked' : '' }}
                                    {{ $role->id === 1 ? 'disabled' : '' }}>
                                @php
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.module-checkbox').forEach(function(checkbox) {
            ...
            const allPerms = document.querySelectorAll('.module-' + module);
        ...
        document.querySelectorAll('.module-checkbox').forEach(function(checkbox) {
            checkbox.addEventListener('change', function() {
                ...
                perms.forEach(function(p) {
                    if(!p.disabled) p.checked = checkbox.checked;
                });
            });
        });
        ...
        if(selectAll) {
            selectAll.addEventListener('click', function() {
                document.querySelectorAll('.perm-checkbox:not(:disabled)').forEach(c => c.checked = true);
                document.querySelectorAll('.module-checkbox:not(:disabled)').forEach(c => { c.checked = true; c.indeterminate = false; });
            });
        }
        ...
        if(deselectAll) {
            deselectAll.addEventListener('click', function() {
                document.querySelectorAll('.perm-checkbox:not(:disabled)').forEach(c => c.checked = false);
                document.querySelectorAll('.module-checkbox:not(:disabled)').forEach(c => { c.checked = false; c.indeterminate = false; });
            });
        }
    });
</script>
